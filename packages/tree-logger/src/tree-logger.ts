...
import mapStringToLast2Characters from './utils/map-string-to-last-2-characters';
import mapStringToShorten2Characters from './utils/map-string-to-shorten-2-characters';
  ...
  private readonly logItem = (...indices: readonly number[]): number => {
    ...
    if (errorsCount > NONE) {
      ...
      for (let i = 0; i < errorsCount; i++) {
        const error: Error = item.errors[i];
